...
4.9.7 不冒险重写以下非阻断项：`LlmService` 仍保留一套低覆盖的旧 Agent 循环；WinUI 渲染缺少独立自动化测试项目；SQLite 仍采用前向型轻量迁移而非正式 EF migrations；Core 中少数大型服务职责过宽。建议 5.0 前先拆除旧循环并为更新、工具安全、后台任务和 UI parser 设置覆盖率增长门槛。
